...
Two modes throughout challenge → match → result: `'race'` (first to a distance, `distance_m`) and `'timed'` (most metres before the clock, `duration_ms`). Any mode-sensitive code must handle both. Two ways in, both landing in `beginMatch`: a direct challenge, where the full menu of distances and durations is on offer because you know who you are asking; or the quick-match queue, which runs exactly the two fixed `QUICK_FORMATS` (`server/lib/validate.js`, served over `/api/meta` so the client only carries the words) because a random opponent agrees to a format, not a negotiation. Queueing is the consent — there is no challenge row and nothing to accept. The queue is in memory only, keyed per player, and `tryPair` runs both on join and from the 15 s sweeper, because whether two runners fit changes when one of them moves; the same sweeper expires searches older than `QUEUE_TTL_MS`. Server-side rules the client must not be trusted with: progress is monotonic and capped (finish line for races, max-plausible-speed for timed), timed duels settle themselves via a server deadline timer (plus a grace for the final progress frame), a disconnected runner gets a 45 s forfeit grace, and `reconcileOnBoot` abandons matches left `live` by a dead process without settling ratings.
